...
def getpixel():
    ...
    img=result_norm
    kk = np.zeros((int(np.ceil(img.shape[0]/20)),int(np.ceil(img.shape[1]/20)), 3))
    height=int(np.ceil(img.shape[0]/20))
    width=int(np.ceil(img.shape[1]/20))
    np.set_printoptions(threshold=sys.maxsize)
    s = str(kk.shape[0]).zfill(3)+str(kk.shape[1]).zfill(3)+"xy" #* (width*height)
    for i in range(3):
        one = img[:, :, i]
        kk[:, :, i] = skimage.measure.block_reduce(one, (20, 20), np.min)
    ans=np.zeros((int(np.ceil(img.shape[0]/20)),int(np.ceil(img.shape[1]/20))))
